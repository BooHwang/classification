...
class FocalLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        BCE_loss = F.binary_cross_entropy_with_logits(outputs, targets.float(), reduce=False)
...
def fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            loss_value  = FocalLoss()(outputs, targets)
            loss_value.backward()
            optimizer.step()
        else:
    ...
    if local_rank == 0:
        pbar.close()
        print(f'accuracy: {val_accuracy / (iteration + 1)}')
