...
def evaluteTop1_5(classfication, lines, metrics_out_path):
    ...
    for index, line in tqdm(enumerate(lines), total=total):
        ...
        if y == 0 and pred_1 != y:
            path = os.path.join('incorrect_class/adult-child', f'{pred[1]:.3f}_{os.path.split(annotation_path)[-1]}')
            x.save(path)
        elif y == 1 and pred_1 != y:
            path = os.path.join('incorrect_class/child-adult', f'{pred[1]:.3f}_{os.path.split(annotation_path)[-1]}')
            x.save(path)

        # if pred_1 == 0:
        #     path = os.path.join('class/adult', f'{pred[0]:.3f}_{os.path.split(annotation_path)[-1]}')
        #     x.save(path)
        # elif pred_1 == 1:
        #     path = os.path.join('class/child', f'{pred[1]:.3f}_{os.path.split(annotation_path)[-1]}')
        #     x.save(path)
...
def show_results(miou_out_path, hist, Recall, Precision, name_classes, tick_font_size = 12):
    ...
    print("mRecall = {0:.2f}%".format(np.nanmean(Recall)*100), "mPrecision = {0:.2f}%".format(np.nanmean(Precision)*100))
